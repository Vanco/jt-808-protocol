...
public class MsgDecoder {
    ...
    public String bodyToString(PackageData packageData) {
    ...
    private void expend0f3b(StringBuilder sb, byte[] data) {
        ...
        for (int i = 0, offset = 0; i < pkgNum; i++) {
            ...
            for (int j = 0; j < pkgNum2; j++) {
                ...
                int status = this.parseIntFromBytes(data, 56 + offset + p, 4);
                sb.append("\t\tStatus Flag:").append(status).append("\n");
                sb.append("\t\t\tbit0 [1:ACC] = ").append(bitOperator.getBitAt(status, 0)).append("\n");
                sb.append("\t\t\tbit1 [1:定位标志] = ").append(bitOperator.getBitAt(status, 1)).append("\n");
                sb.append("\t\t\tbit2 [1:重传包，0:正常包] = ").append(bitOperator.getBitAt(status, 2)).append("\n");
                sb.append("\t\t\tbit3 [1:金融锁车激活] = ").append(bitOperator.getBitAt(status, 3)).append("\n");
                sb.append("\t\t\tbit4 [1:金融锁车状态] = ").append(bitOperator.getBitAt(status, 4)).append("\n");
                sb.append("\t\t\tbit5 [1:GNSS 模块发生故障] = ").append(bitOperator.getBitAt(status, 5)).append("\n");
                sb.append("\t\t\tbit6 [1:终端主电源欠压] = ").append(bitOperator.getBitAt(status, 6)).append("\n");
                sb.append("\t\t\tbit7 [1:碰撞报警] = ").append(bitOperator.getBitAt(status, 7)).append("\n");
                sb.append("\t\t\tbit8 [1:Can 收发器故障] = ").append(bitOperator.getBitAt(status, 8)).append("\n");
                sb.append("\t\t\tbit9 [1:Wifi 模块故障] = ").append(bitOperator.getBitAt(status, 9)).append("\n");
                sb.append("\t\t\tbit10[1:无线网络模块异常] = ").append(bitOperator.getBitAt(status, 10)).append("\n");
    ...
    private void expend0f3a(StringBuilder sb, byte[] data) {
        sb.append("Version:").append("\n");
        int v = this.parseIntFromBytes(data, 0, 1);
    ...
    private void expend9102(StringBuilder sb, byte[] data) {
    ...
    private void expend0f51(StringBuilder sb, byte[] data) {
    ...
    private void expend8f51(StringBuilder sb, byte[] data) {
        sb.append("Version:").append(this.parseIntFromBytes(data, 0, 1)).append("\n");
        sb.append("Time:").append(this.parseBcdStringFromBytes(data, 1, 6)).append("\n");
    ...
    private void expend0f41(StringBuilder sb, byte[] data) {
        sb.append("Version:").append(this.parseIntFromBytes(data, 0, 1)).append("\n");
        sb.append("Time:").append(this.parseBcdStringFromBytes(data, 1, 6)).append("\n");
    ...
    private void expend8f41(StringBuilder sb, byte[] data) {
        sb.append("Version:").append(this.parseIntFromBytes(data, 0, 1)).append("\n");
        sb.append("Time:").append(this.parseBcdStringFromBytes(data, 1, 6)).append("\n");
        int cmdLen = this.parseIntFromBytes(data, 7, 1);
        sb.append("Commands:").append(cmdLen).append("\n");
    ...
    private String lightIdToName(byte lid) {
        switch (lid) {
            ...
            case 4:
                return "TurnRight:";
            case 5:
                return "HazardFlasher:";
            case 6:
                return "IdentificationLamp:";
            case 7:
                return "BrakeLamp:";
            case 8:
                return "FrontFogLamp:";
